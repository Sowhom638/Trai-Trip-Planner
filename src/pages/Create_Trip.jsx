...
export default function Create_Trip() {
  ...
  const [userInfo, setUserInfo] = useState(null);
    ...
    const userData = usersList && usersList?.users?.length>0 && usersList?.users?.find((user)=>user?.email===userInfo?.email);
  const GetUserProfile = (tokenInfo) => {
    ...
    }).catch((error)=>setgoogleError(error)).finally(()=>setShowPopup(false));
  };
  ...
  return (
    <>
      ...
      <div className="sm:px-10 md:px-32 lg:px-56 xl:px-72 px-5 mt-10">
        ...
        <div>
        {itineraryData && (
          <TripDetails itineraryData={itineraryData} />
        )}
        </div>
        <Popup isOpen={showPopup} onClose={() => setShowPopup(false)} login={login} />
